    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-primary">
                ...
                <div class="card-body">
                    <form method="post" action="{{route('admin.sales.edit.update',$sale->id)}}">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="customer" name="customer_id" class="form-control select2">
                                        ...
                                        <option value="{{$customer->id}}" @if ($customer->id == $sale->customer_id)
                            ...
                            <div class="col-md-12">
                                ...
                                <table id="dynamic-product-list" class="w-100 table table-bordered">
                                    ...
                                    <tr>
                                        <td id="product-id-{{$key}}" class="product-id">
                                            <select id="productId-{{$key}}" name="products[{{$key}}][name]"
                                                class="form-control select2">
                                                @foreach($products as $product)
                                                <option value="{{$product->id}}" @if ($product->id ==
                                                    $item->product_id) selected
                                                    @endif data-price="{{$product->price}}">{{$product->name}}
                                                </option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                                            <input type="number" id="inputQuantity-{{$key}}"
                                                name="products[{{$key}}][quantity]" class="form-control"
                                                value="{{$item->quantity}}">
                                        </td>
                                        <td>
                                            @if ($key == 0)
                                            ...
                                            @else
                                            <button type="button"
                                                class="btn btn-danger remove-input-field w-100">Delete</button>
                                            @endif
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea id="inputNote" name="note"
                                        ...
                                        row="4">{{old('note', $sale->note ?? '')}}</textarea>
                                    @error('note')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
...
<script type="text/javascript">
    ...
    `<tr>
        ...
        <td>
            <input type="number" id="inputQuantity-`+ i +`" name="products[`+ i +`][quantity]" class="form-control" value="1">
        </td>
        <td>
