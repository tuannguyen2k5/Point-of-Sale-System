        ...
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Store #{{$store->name}}</h3>
                </div>
                <div class="card-body">
                    <form method="post" action="{{route('admin.stores.edit.update',$store->id)}}">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select id="warehouse" name="warehouse_id" class="form-control select2">
                                ...
                                <option value="{{$warehouse->id}}" @if ($warehouse->id == $store->warehouse_id)
                                    checked @endif>{{$warehouse->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select id="manager" name="manager_id" class="form-control select2">
                                ...
                                <option value="{{$manager->id}}" @if ($manager->id == $store->manager_id)
                                    checked @endif>{{$manager->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
